...
const buildCourseDetailPath = (courseId: number, level?: string | null) => {
    const normalizedLevel = (level || "").trim().toUpperCase();
    const mappedSlug = levelSlugMap[normalizedLevel];
    const levelSlug = mappedSlug || "a1-almanca-kursu";
    return `/kurslar/${levelSlug}/${courseId}`;
};
...
export default function FaceToFaceCoursesPage() {
    const router = useRouter();
    const dispatch = useAppDispatch();

    const courseTypeParam = "face_to_face";
    const fetchFaceToFaceCourses = useGetCourseListQuery({
        page: 1,
        ordering: "-created_at",
        type: courseTypeParam,
    });
    const faceToFaceCourses = fetchFaceToFaceCourses.data?.results || [];
    const { isLoading, error } = fetchFaceToFaceCourses;
    ...
    const handleAddToCart = (courseItem: (typeof faceToFaceCourses)[number]) => {
    ...
    const handleBuyNow = (courseItem: (typeof faceToFaceCourses)[number]) => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            <main className="grow max-w-7xl mx-auto w-full px-4 py-12 md:py-20 mt-16">
                ...
                ) : faceToFaceCourses.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-slate-200">
                        <p className="text-xl text-slate-500 font-medium">Şu anda aktif yüz yüze kurs bulunmamaktadır.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 w-full pt-6 pb-12">
                        {faceToFaceCourses.map((courseItem) => {
                                ...
                                <Card key={courseItem.id} className="h-full flex flex-col bg-white rounded-[32px] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden">
                                    <div className="relative aspect-16/10 w-full overflow-hidden bg-slate-100">
                                        ...
                                        <div className="absolute top-4 left-4 flex gap-2">
                                            <Badge className="bg-white/95 text-[#1A3EB1] hover:bg-white font-bold px-3 py-1 text-xs border-0 shadow-sm backdrop-blur-sm">
                                                Yüz Yüze
                                            </Badge>
                                    ...
                                    <CardContent className="p-6 flex-1 flex flex-col">
                                        ...
                                        <div className="mt-auto pt-5 border-t border-slate-50">
                                            ...
                                            <div className="grid grid-cols-1 gap-2.5">
                                                <Link href={buildCourseDetailPath(courseItem.id, courseItem.level)} className="w-full">
